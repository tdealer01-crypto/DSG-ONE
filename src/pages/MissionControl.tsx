...
import { Activity, Server, Shield, Zap, AlertTriangle, CheckCircle2, Clock, Target, ShieldAlert, FileCheck, History } from "lucide-react";

function MetricCard({ title, value, sub, icon, status }: { title: string, value: string, sub?: string, icon?: React.ReactNode, status?: "success" | "warning" | "danger" | "neutral" }) {
  const statusColors = {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 h-full flex flex-col">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 shrink-0">
        ...
        <MetricCard title="Decision Mix" value="92%" sub="ALLOW rate" icon={<Target size={14} />} status="success" />
        <MetricCard title="Core Integrity" value="100%" sub="Verified" icon={<Shield size={14} />} status="success" />
        <MetricCard title="Alerts" value="2" sub="Requires review" icon={<AlertTriangle size={14} />} status="warning" />
        <MetricCard title="Policy Pressure" value="Low" sub="Normal ops" icon={<Zap size={14} />} status="success" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-4 bg-card border border-border rounded-xl p-4 flex flex-col h-full">
          ...
          <div className="flex-1 bg-secondary/20 border border-border/50 rounded-lg relative overflow-hidden flex items-center justify-center">
            ...
            <div className="relative w-full h-full p-4 flex flex-col justify-between">
              ...
              <div className="flex justify-center">
                <div className="w-16 h-16 rounded-full border-2 border-primary bg-primary/10 flex items-center justify-center shadow-[0_0_20px_rgba(20,184,166,0.4)]"><Shield size={24} className="text-primary" /></div>
              </div>
              <div className="flex justify-between">
        ...
        <div className="lg:col-span-8 bg-card border border-border rounded-xl flex flex-col h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-auto">
            <table className="w-full text-xs text-left">
              <thead className="text-[10px] text-muted-foreground uppercase bg-secondary/30 sticky top-0 backdrop-blur-md">
                <tr>
              ...
              <tbody className="divide-y divide-border/50">
                {decisionStream.map((d, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">{d.req}</td>
