  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4 flex flex-col justify-between">
      ...
      <div>
        ...
        {sub && (
          <div
            className={`text-[10px] mt-1 font-medium ${status ? statusColors[status] : "text-muted-foreground"}`}
          >
            {sub}
          </div>
        )}
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 h-full flex flex-col">
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Real-time operational overview of the DSG ONE control plane.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 shrink-0">
        ...
        <MetricCard
          title="Alerts"
          value={ledger.filter((l) => l.decision === "STABILIZE").length + 2}
          sub="Requires review"
        ...
        <MetricCard
          ...
          value={
            ledger.filter((l) => l.decision === "BLOCK").length > 0
              ? "High"
              : "Low"
          }
          sub="Recent blocks"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 min-h-0">
                ...
                <div className="flex items-center justify-between border-t border-border/50 pt-3">
                  ...
                  <div
                    className={`text-xs font-bold px-2 py-1 rounded ${
                      activeProposal.decision === "ALLOW"
                        ? "bg-success/20 text-success"
                        : activeProposal.decision === "STABILIZE"
            ...
            <div className="flex-1 bg-secondary/20 border border-border/50 rounded-lg relative overflow-hidden flex items-center justify-center z-10">
              ...
              <div className="relative w-full h-full p-4 flex flex-col justify-between">
                <div className="flex justify-between">
                  <div className="w-12 h-12 rounded-full border-2 border-success bg-success/10 flex items-center justify-center animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.3)]">
                    <Server size={16} className="text-success" />
                  </div>
                  <div className="w-12 h-12 rounded-full border-2 border-warning bg-warning/10 flex items-center justify-center shadow-[0_0_15px_rgba(234,179,8,0.3)]">
                    <Server size={16} className="text-warning" />
                  </div>
                </div>
                <div className="flex justify-center">
                  <div
                    className={`w-16 h-16 rounded-full border-2 ${state !== "IDLE" ? "border-primary bg-primary/20 shadow-[0_0_30px_rgba(20,184,166,0.6)] animate-pulse" : "border-primary bg-primary/10 shadow-[0_0_20px_rgba(20,184,166,0.4)]"} flex items-center justify-center transition-all duration-500`}
                  >
                    <Shield size={24} className="text-primary" />
                  </div>
                </div>
                <div className="flex justify-between">
                  <div className="w-12 h-12 rounded-full border-2 border-success bg-success/10 flex items-center justify-center">
                    <Server size={16} className="text-success" />
                  </div>
                  <div className="w-12 h-12 rounded-full border-2 border-destructive bg-destructive/10 flex items-center justify-center shadow-[0_0_15px_rgba(239,68,68,0.3)]">
                    <Server size={16} className="text-destructive" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-4 bg-card border border-border rounded-xl flex flex-col overflow-hidden">
          <div className="p-4 border-b border-border shrink-0">
            <h3 className="text-sm font-semibold flex items-center gap-2 uppercase tracking-wider text-muted-foreground">
              <FileCheck size={14} className="text-primary" /> Proof & Ledger
              Queue
            </h3>
          </div>
          <div className="flex-1 overflow-auto p-4 space-y-3">
